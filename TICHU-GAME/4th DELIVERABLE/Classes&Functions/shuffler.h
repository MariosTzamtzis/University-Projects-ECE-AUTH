...
template <class X> void cut(X** myArray, int myArraySize){
    ...
    if (cutPoint != 0 && cutPoint != myArraySize-1){
        ...
        for (i=cutPoint, j=0; i<myArraySize; i++, j++) myArray[j]=myArray[i]; ///Transport the last part to the beginning of the deck.
        for (i=0; j<myArraySize; i++, j++) myArray[j]=temp[i]; ///Transport the first part to the end of the deck.
