...
public class Player {
	...
	public int[] move(int id,int dice){//10: down, 11: left, 12:up, 13: right
		int[] array=new int [4];
		
		//Matching the dice from heuristicPlayer with this' function dice 
		if(dice==1)//up
			dice=12;
		else if(dice==3)//right
			dice=13;
		else if(dice==5)//down
			dice=10;
		else if(dice==7)//left
		...
		if(dice==10) {
			...
			if(board.getTiles()[id].getDown()==true) {
				...
				array[3]=-1;//supply-id
				//Printing Theseus' score
				if(name=="Theseus")
				System.out.println("Total supplies' collected by Theseus: " + score);
				return array;
			}
