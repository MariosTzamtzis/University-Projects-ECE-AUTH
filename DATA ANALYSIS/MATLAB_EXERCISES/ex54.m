...
air_data = importdata("lightair.dat");
...
[b,bint] = regress(Y,[ones(n,1) X]);%returns a vector b of coefficient estimates for a multiple linear regression of the responses in vector y on the predictors in matrix X. To compute coefficient estimates for a model with a constant term (intercept), include a column of ones in the matrix X.
% Least squares error               %returns a matrix bint of 95%
% confidence intervals for the coefficient estimates,
% bint(1,1),bint(1,2) for intercept,
% bint(2,1),bint(2,2) for regression coefficient
...
Sxx = varX*(n-1);
sb0 = se * sqrt(1/n + (x0-mX)^2/Sxx);
cib0 = [y0-tc*sb0 y0+tc*sb0];
...
x0 = linspace(min(X),max(X),100)';%returns a row vector of 100 evenly spaced points between min(x) and max(x).
y0 = b(2)*x0 + b(1);
sb0 = se * sqrt(1/n + (x0-mX).^2/Sxx);
cib0 = [y0-tc*sb0 y0+tc*sb0];
y0std = se*sqrt(1+1/n+(x0-mX).^2/Sxx);
