...
% Define the function f(u1,u2)
syms f(u1,u2)
f(u1,u2) = sin(u1+u2) * sin(u2^2);

% Define the limits for u1 and u2
u1_limits = [-1, 2];
u2_limits = [-2, 1];
...
c = 30;     % number of pairs of chromosomes to be crossed over
m = 30;     % number of chromosomes to be mutated
ng = 300;  % number of generations
% -----------------------------------------------------------------

% Data sampling 
% Generate test input X
X = zeros(256,2);
for i = 1:16:256
...
temp = 1;
% Generate test input X continued
for i = 0:15
    X(i+temp:temp+15+i,2) = -2 + i*(0.2);
    temp = temp + 15;
end

% Generate expected output y_test using function f(u1,u2)
y_test = zeros(256,1);
for i = 1:256
...
% Genetic Algorithm Loop: Crossover -> Mutation -> New Population -> Evaluation -> Selection
for i = 1:ng
    Cr = crossover(P,c);
    Mu = mutation(P,m);
    P(p+1:p+2*c,:) = Cr;
...
% Extract the best solution (best chromosome)
solution = P(1,:);

% Decode the best chromosome to obtain parameters a, c1, c2, s1, s2, and b
a = zeros(15,1); c1 = zeros(15,1); c2 = zeros(15,1);
s1 = zeros(15,1); s2 = zeros(15,1); b = 0;
temp1 = 1; temp2 = 1;

for i = 1:15
    % Decoding parameters from binary representation
...
% Plotting
% Plot the original function f(u1, u2)
figure(1)
...
% Plot the estimated function surface based on the best chromosome
figure(2)
